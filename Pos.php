...
class Pos extends GrabPay
{
	...
	/**
	 * Performs a payment transaction which charges from the wallet associated with the request QR code and payouts to request merchant grabID.
	 *
	 * @param string $txId order ID
	 * @param int $amount ​Transaction amount as integer
	...
	public function qrCodeInquiry(string $txId): object
	{
		...
		return $this->callGrabApi('GET', $requestUri, [
			'Date' => $date,
			'Authorization' => $this->partnerId . ':' . $hmacSignature,
		], $data);
	}
	...
	public function cancelTransaction(string $origTxID): object
	{
		...
		return $this->callGrabApi('PUT', $requestUri, [
			'Date' => $date,
			'Authorization' => $this->partnerId . ':' . $hmacSignature,
		], $data);
	}
